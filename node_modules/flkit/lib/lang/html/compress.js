...
var HtmlCompress = function (_Base) {
  ...
  HtmlCompress.prototype.compressCommon = function compressCommon(token) {
    ...
    if (token.commentBefore.length && this.options.removeComment) {
      (function () {
        ...
        if (_this2.prev && _this2.prev.type === _token_type2.default.HTML_TEXT) {
          prevHasRightSpace = /\s$/.test(_this2.prev.value);
        }
  ...
  HtmlCompress.prototype.compressTagStart = function compressTagStart(token) {
    ...
    attrs.forEach(function (attr) {
      if (attr.type === _token_type2.default.TPL) {
        var tplCompress = _this3.compressTpl(attr);
        if (tplCompress) {
      ...
      if (name === 'class' && !_this3.hasTpl(value)) {
      ...
      if (options.compressStyleValue && name === 'style') {
        value = '*{' + value + '}';
        if (_this3.cssHandle && _this3.cssHandle.compress) {
          var compressValue = _this3.cssHandle.compress(value);
          attr.value = compressValue.slice(2, compressValue.length - 1);
        } else {
          var instance = new _compress2.default(value, _this3.options);
  ...
  HtmlCompress.prototype.compressScript = function compressScript(token) {
    ...
    var start = _token$ext.start;
    var content = _token$ext.content;
    var end = _token$ext.end;
  ...
  HtmlCompress.prototype.run = function run() {
    var retTokens = arguments.length <= 0 || arguments[0] === undefined ? false : arguments[0];
