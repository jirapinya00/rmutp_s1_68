...
var CssTokenize = function (_BaseTokenize) {
    ...
    switch (code) {
      ...
      case 0x3a:
        ...
        if (type === _token_type2.default.CSS_PROPERTY || type === _token_type2.default.CSS_SELECTOR || type === _token_type2.default.CSS_VALUE || type === _token_type2.default.CSS_COLON) {
          var _token = this.getToken(_token_type2.default.CSS_COLON, this.next());
      ...
      case 0x3b:
        // ;
        var token = this.getToken(_token_type2.default.CSS_SEMICOLON, this.next());
  ...
  CssTokenize.prototype.getSelectorToken = function getSelectorToken() {
    ...
    if (this.options.parseSelector) {
      token.ext = new _selector_tokenize2.default(token.value, this.options).run();
    }
    return token;
  };
  /**
  ...
  CssTokenize.prototype.run = function run() {
    ...
    for (; token = this.getNextToken();) {
      if (token.type === _token_type2.default.CSS_LEFT_BRACE && ret.length && ret[ret.length - 1].type === _token_type2.default.TPL) {
        var prev = ret[ret.length - 1];
        prev.type = _token_type2.default.CSS_SELECTOR;
        prev.ext = {};
        this.prevToken = prev;
        this.status = STATUS.PROPERTY;
      }
      //  merge when prev token is tpl
      else if (token.type === _token_type2.default.CSS_SELECTOR && ret.length && ret[ret.length - 1].type === _token_type2.default.TPL) {
          var _prev = ret[ret.length - 1];
          var space = '';
          if (_prev.end !== token.start) {
            space = ' ';
          }
          token.value = _prev.value + space + token.value;
          token.start = _prev.start;
          token.loc.start = _prev.loc.start;
          if (token.ext.group) {
            token.ext.group[0].tokens[0].value = _prev.value + space + token.ext.group[0].tokens[0].value;
          }
